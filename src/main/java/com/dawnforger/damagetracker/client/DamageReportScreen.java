...
public final class DamageReportScreen extends Screen {
    ...
    public void render(GuiGraphics gg, int mouseX, int mouseY, float partialTick) {
        ...
        int visible = (this.height - y - 20) / LINE_HEIGHT;
        ...
        if (rows.size() > visible) {
            gg.drawString(font, String.format("Scroll: %d/%d", scroll/1 + 1, Math.max(1, (rows.size()+visible-1)/visible)), x, this.height - 16, 0xFFAAAAAA, false);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        int visible = (this.height - 20 - 20 - 12*3) / LINE_HEIGHT;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        // Use InputConstants.getKey(...) to build the Key instance expected by isActiveAndMatches
        if (Keybinds.OPEN_REPORT != null && Keybinds.OPEN_REPORT.isActiveAndMatches(InputConstants.getKey(keyCode, scanCode))) {
            onClose();
            return true;
        }
